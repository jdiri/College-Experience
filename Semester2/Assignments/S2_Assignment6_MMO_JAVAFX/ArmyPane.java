// Assignment #: 6
//  Description: creates the armypane tab
...
public class ArmyPane extends BorderPane {
	...
	public ArmyPane(ArrayList<Hero> heroList) {
		...
		armyInfo = new Label();
		checkBox = new VBox();
		load = new Button("Load Heroes/Clear Selection");
		// ^^^^^^ 5. a) ^^^^^^

		// TODO: 5. b) Bind "Load Heroes/Clear Selection" Button to its handler
		// vvvvvv 5. b) vvvvvv (1 line)
		load.setOnAction(new LoadHeroesButtonHandler());
		// ^^^^^^ 5. b) ^^^^^^
		...
		setCenter(checkBox);setTop(armyInfo);setBottom(load);
		// ^^^^^^ 5. c) ^^^^^^
	...
	private class LoadHeroesButtonHandler implements EventHandler<ActionEvent> {
		...
		public void handle(ActionEvent event) {
			...
			for(int i = 0; i<heroList.size(); i++) {
				CheckBox list = new CheckBox(heroList.get(i).toString());
				list.setOnAction(new CheckBoxHandler(heroList.get(i)));
				checkBox.getChildren().add(list);
			}
			// ^^^^^^ 6. b), c), d) ^^^^^^
	...
	private class CheckBoxHandler implements EventHandler<ActionEvent> {
		...
		public void handle(ActionEvent event) {
			...
			CheckBox list = (CheckBox) event.getSource();
			// ^^^^^^ 7. a) ^^^^^^
			...
			if(list.isSelected()) {
				...
				totalCharisma = totalCharisma+hero.getCharisma();
